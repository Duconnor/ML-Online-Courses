...
#define STOP_RECORD_TIMER(name) {                           \
    ...
    for (int ch = 0; ch < nChannels; ch++){
        ...
        cudaMemset(dev_input_data, 0, sizeof(cufftComplex) * padded_length); // Padding before we copy the data.
       ...
       cudaMemset(dev_impulse_v, 0, sizeof(cufftComplex) * padded_length); // Padding before we copy the data.
       ...
       // Already DONE that before copying data from host to device.
       ...
       // DONE
        ...
        cudaMemcpy(output_data, dev_out_data, sizeof(float) * padded_length, cudaMemcpyDeviceToHost);
...
int main(int argc, char **argv){
    ...
    int max_time_allowed_in_seconds = 900;
