    ...
    int n, int c, int h, int w)
{
    ...
    int globalmem_idx = threadIdx.x + blockIdx.x * blockDim.x;
    const int shmem_idx = threadIdx.x;

    // Initialize the shared memory
    shmem[shmem_idx] = 0.0;
    __syncthreads();

    // Perform the log and addition operations and save the results in the shared memory
    while (globalmem_idx < n * c * h * w) {
        shmem[shmem_idx] += -(log(pred_Y[globalmem_idx]) * true_Y[globalmem_idx]);
        globalmem_idx += blockDim.x * gridDim.x;
    }
    __syncthreads();

    // Perform reduction
    int shmem_threshold = blockDim.x;
    for (int i = 0; i < (int)log2((double)blockDim.x); i++) {
        shmem_threshold /= 2;
        if (shmem_idx < shmem_threshold) {
            shmem[shmem_idx] += shmem[shmem_idx + shmem_threshold];
        }
        __syncthreads();
    }
    // DONE
