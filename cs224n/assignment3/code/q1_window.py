...
class Config:
    ...
    n_window_features = n_word_features * (window_size * 2 + 1)
    # END YOUR CODE
    n_classes = 5
    dropout = 0.5
    embed_size = 50
...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        ...
        for idx, word_label in enumerate(zip(sentence, labels)):
            word, label = word_label
            before, after = [], []
            num_start = max([window_size - idx, 0])
            num_before = window_size - num_start
            before = [start] * num_start + sentence[idx - num_before:idx]
            num_after = window_size if window_size + \
                idx < len(sentence) else len(sentence) - idx - 1
            num_end = window_size - num_after
            after = sentence[idx + 1:idx + num_after + 1] + [end] * num_end
            windowed_data.append(
                (reduce(lambda x, y: x + y, before + [word] + after), label))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(
            E, self.input_placeholder), (-1, self.config.n_window_features * self.config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W', shape=(self.config.embed_size * self.config.n_window_features, self.config.hidden_size),
                            initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable('b1', shape=(
            self.config.hidden_size,), initializer=tf.zeros_initializer())
