...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        import copy
        sentence_prime, labels_prime = copy.deepcopy(sentence), copy.deepcopy(labels)
        pad_length = max([max_length - len(sentence), 0])

        if max_length < len(sentence):
            sentence_prime = copy.deepcopy(sentence[:max_length])
            labels_prime = copy.deepcopy(labels[:max_length])

        zero_vectors = [zero_vector for _ in range(pad_length)]
        zero_labels = [zero_label for _ in range(pad_length)]
        sentence_prime += zero_vectors
        labels_prime += zero_labels
        mask = [True] * (max_length - pad_length) + [False] * pad_length
        ret.append((sentence_prime, labels_prime, mask))
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(E, self.input_placeholder), shape=(
    ...
    def add_prediction_op(self):
        ...
        h = tf.zeros((tf.shape(self.input_placeholder)[0], self.config.hidden_size))
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=tf.boolean_mask(self.labels_placeholder, self.mask_placeholder), logits=tf.boolean_mask(preds, self.mask_placeholder))
