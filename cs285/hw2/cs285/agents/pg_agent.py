...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        discounts = [self.gamma ** idx for idx in indices]

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = [gamma * r for gamma,
                              r in zip(discounts, rewards)]

        # 4) calculate a scalar: sum_{t'=0}^{T-1} gamma^(t') * r_{t'}
        import functools
        sum_of_discounted_rewards = functools.reduce(
            lambda x, y: x + y, discounted_rewards)

        # 5) create a list of length T-1, where each entry t contains that scalar
        list_of_discounted_returns = np.array([sum_of_discounted_rewards] * T)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)):
            ...
            discounts = [self.gamma ** (idx - start_time_index)
                         for idx in indices]
            ...
            discounted_rtg = [
                gamma * r for gamma, r in zip(discounts, rewards[start_time_index:])]

            # 4) calculate a scalar: sum_{t'=t}^{T-1} gamma^(t'-t) * r_{t'}
            import functools
            sum_discounted_rtg = functools.reduce(
                lambda x, y: x + y, discounted_rtg)
