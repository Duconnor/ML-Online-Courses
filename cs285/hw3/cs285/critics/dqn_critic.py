...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        if self.double_q:
            ...
            double_q_tp1_values = q_func(
                self.obs_tp1_ph, self.ac_dim, scope='q_func', reuse=tf.AUTO_REUSE)
            max_each_row_indices = tf.cast(
                tf.argmax(double_q_tp1_values, axis=1), tf.int32)
            indices = tf.stack([tf.range(tf.shape(q_tp1_values)[
                               0], dtype=tf.int32), max_each_row_indices], axis=1)
